...
# Add other data variables
# Add other data variables
theta_dot_lwr_cs = theta_dot_lwr.sel(time=time_cs,  method='nearest').sel(longitude=slice(lon_min_cs, lon_max_cs),    level=slice(lev_min_cs, lev_max_cs),
                                                     latitude=slice(lat_max_cs, lat_min_cs)).mean('latitude', skipna = False)
im = theta_dot_lwr_cs.plot.contourf(ax=ax2, add_colorbar=False, levels = np.arange(-1.2,1.21, 0.2), extend = 'both' )
# im = theta_dot_lwr.sel(time=time_cs).sel(longitude=slice(lon_min_cs, lon_max_cs),    level=slice(lev_min_cs, lev_max_cs)).sel(latitude=lat_max_cs, method = 'nearest').plot.contourf(
