...
time = np.arange(0, all_data['PV_dot_friction_1'].shape[0])
...
for i in range(np.size(traj['n_seeds'])):
    plt.plot(t_list, PV_dot_friction_list[i] * 1e6 * 3600, alpha = 0.3)
plt.xlabel('Time')
plt.ylabel(r'$ \dot{PV}_{friction}$ [pvu.h$^{-1}$]')
plt.title(f'{cyc}')
plt.show()

Fy = all_data_pressure['Fy'] * 3600
...
im = Fy.sel(time=time_cs, level= pres).plot.contourf(
    ax=ax1, add_colorbar=False,levels = np.arange(-6, 6.1, 1), extend = 'both', transform=ccrs.PlateCarree() )

traj0 = traj.isel(time_ind = time_ind)#.sel(n_seeds =n_seeds_heating_lwr_neg)
PV_dot_friction0 = PV_dot_friction_list[:, time_ind]
traj0 = traj0.where(PV_dot_friction0 * 3600 * 1e6 <= threshold )
...
ax1.plot([lon_min_cs, lon_min_cs, lon_max_cs, lon_max_cs, lon_min_cs], [lat_min_cs, lat_max_cs, lat_max_cs, lat_min_cs, lat_min_cs], color = 'red', transform=ccrs.Geodetic(), linewidth = 2)
...
# Add other data variables
# Add other data variables
Fy_cs = Fy.sel(time=time_cs).sel(longitude=slice(lon_min_cs, lon_max_cs),    level=slice(lev_min_cs, lev_max_cs),
                                                     latitude=slice(lat_max_cs, lat_min_cs)).mean('latitude', skipna = False)
im = Fy_cs.plot.contourf(ax=ax2, add_colorbar=False, levels = np.arange(-6, 6.1, 1), extend = 'both' )
# im = Fy.sel(time=time_cs).sel(longitude=slice(lon_min_cs, lon_max_cs),    level=slice(lev_min_cs, lev_max_cs)).sel(latitude=lat_max_cs, method = 'nearest').plot.contourf(
#     ax=ax2, add_colorbar=False, levels = np.arange(-6, 6.1, 1), extend = 'both' )
...
CSt = theta.sel(time=time_cs,  method='nearest').sel(longitude=slice(lon_min_cs,lon_max_cs),level=slice(lev_min_cs, lev_max_cs),
                                                                    latitude=slice(lat_max_cs, lat_min_cs)).mean('latitude', skipna = False).plot.contour(ax=ax2,
                                                                                                                  add_colorbar=False,
                                                                                                                  colors='green', levels=  np.arange(274,321,4))
...
cbar.set_label(r'$F_y$ [$m.s^{-1}.h^{-1}$]')#, size = 20)
...
plt.savefig(f'{path_plots}/friction_meridional_article_{dataset}_{cyc}.png', bbox_inches='tight', transparent = True)
plt.show()
