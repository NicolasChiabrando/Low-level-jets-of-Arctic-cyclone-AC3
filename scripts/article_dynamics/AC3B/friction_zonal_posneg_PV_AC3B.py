...
from libs.traj import is_in_region, date_from_time_ind
...
time_ind = 4 #np.argwhere(PV_dot_friction_mean == np.max(PV_dot_friction_mean))[0][0]

plt.plot(t_list, PV_dot_friction_mean * 1e6 * 3600, color='black', linewidth=2)
plt.vlines(t_list[time_ind], ymin=np.nanmin(PV_dot_friction_mean) * 1e6 * 3600,
           ymax=np.nanmax(PV_dot_friction_mean) * 1e6 * 3600)
plt.xlabel('Time')
...
ax1.scatter(traj0['lon'], traj0['lat'], color = 'purple', marker = '_', transform=ccrs.PlateCarree(), alpha = 0.5)#, alpha = 0.3)
...
ax2.scatter(traj0['lat'], traj0['P'] / 100, color = 'purple', marker = '_',  alpha = 0.5)#, alpha = 0.3)
...
plt.savefig(f'{path_plots}/friction_posneg_article_ind{time_ind}_{dataset}_{cyc}.png', bbox_inches='tight', transparent = True)
plt.show()
