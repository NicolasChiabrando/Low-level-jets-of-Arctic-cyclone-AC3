...
PV_dot_heating_rad_mean = np.nanmean(PV_dot_heating_SWR_list + PV_dot_heating_LWR_list, axis = 0)
time_ind = np.argwhere(PV_dot_heating_rad_mean == np.min(PV_dot_heating_rad_mean))[0][0]

print(time_ind)
fig0 = plt.figure(0)
plt.plot(t_list, PV_dot_heating_rad_mean* 1e6 * 3600, color = 'black', linewidth = 2)
plt.vlines(t_list[time_ind], ymin = np.nanmin(PV_dot_heating_rad_mean)* 1e6 * 3600, ymax = np.nanmax(PV_dot_heating_rad_mean)* 1e6 * 3600 )
plt.xlabel('Time')
plt.ylabel(r'$PV \ dot_{rad}$ [pvu/h]')
plt.title(f'{cyc}')
plt.show()
...
theta_dot_lwr = all_data_pressure['MTTLWR'] * (100000 / all_data_pressure['level'])**kappa *3600 # in K/s
...
im = theta_dot_lwr.sel(time=time_cs, level= pres, method = 'nearest').plot.contourf(
    ax=ax1, add_colorbar=False, levels =  np.arange(-2,2.1, 0.25), extend = 'both', transform=ccrs.PlateCarree() )
...
im = theta_dot_lwr.sel(time=time_cs, method = 'nearest').sel(longitude=slice(lon_min_cs, lon_max_cs),    level=slice(lev_min_cs, lev_max_cs),
    ...
    ax=ax2, add_colorbar=False, levels =  np.arange(-2,2.1, 0.25), extend = 'both' )
...
cbar.set_label(r'$\dot{\theta}_{long wave}$ [$K.s^{-1}$]')#, size = 20)
...
plt.savefig(f'{path_plots}/lwr_zonal_article_ind{time_ind}_{dataset}_{cyc}.png', bbox_inches='tight', transparent = True)
plt.show()
