...
# print(all_data_pressure['level'].data)
...
n_seeds_heating_neg = []
n_seeds_heating_latent_neg = []
n_seeds_heating_rad_neg = []
n_seeds_heating_lwr_neg = []
n_seeds_heating_swr_neg = []
n_seeds_friction_neg = []
for i in trange(np.size(traj['n_seeds'].to_numpy()), desc="Processing"):
    ...
    delta_PV_friction = np.abs(np.nansum(PV_dot_friction_interp))#, time_seed)
    delta_PV_heating = np.abs(np.nansum(PV_dot_heating_interp))#, time_seed)

    # delta_PV = np.nansum(PV_dot_list[i])
    delta_PV_heating_rad = np.abs(np.nansum(PV_dot_heating_SWR_interp + PV_dot_heating_LWR_interp))
    delta_PV_heating_lwr = np.abs(np.nansum(PV_dot_heating_LWR_interp))
    delta_PV_heating_swr = np.abs(np.nansum(PV_dot_heating_SWR_interp))

    delta_PV_heating_latent =  np.abs(np.nansum(PV_dot_heating_interp) - np.nansum(PV_dot_heating_SWR_interp + PV_dot_heating_LWR_interp)) # delta_PV_heating - delta_PV_heating_rad
    ...
    # delta_PV_heating_lwr = np.nansum(PV_dot_heating_LWR_interp)
    # delta_PV_heating_swr = np.nansum(PV_dot_heating_SWR_interp)
    #
    ...
    #     if delta_PV_heating_latent > delta_PV_heating_lwr :
    #         if delta_PV_heating_latent > delta_PV_heating_swr :
    #             n_seeds_heating_latent.append(i)
    #         else :
    #             n_seeds_heating_swr.append(i)
    #     elif delta_PV_heating_lwr >= delta_PV_heating_swr :
    #         n_seeds_heating_lwr.append(i)
    #     else :
    #         n_seeds_heating_swr.append(i)
    # else:
    #     n_seeds_friction.append(i)

    if delta_PV_heating_latent > delta_PV_friction :
        if delta_PV_heating_latent > delta_PV_heating_lwr :
            if delta_PV_heating_latent > delta_PV_heating_swr :
                n_seeds_heating_latent.append(i)
            else :
                if np.nansum(PV_dot_heating_SWR_interp) > 0:
                    n_seeds_heating_swr.append(i)
                else:
                    n_seeds_heating_swr_neg.append(i)
        elif delta_PV_heating_lwr >= delta_PV_heating_swr :
            if  np.nansum(PV_dot_heating_LWR_interp) > 0 :
                n_seeds_heating_lwr.append(i)
            else :
                n_seeds_heating_lwr_neg.append(i)
        else :
            if np.nansum(PV_dot_heating_SWR_interp) > 0:
                n_seeds_heating_swr.append(i)
            else:
                n_seeds_heating_swr_neg.append(i)
    elif delta_PV_friction > delta_PV_heating_lwr :
        if delta_PV_friction > delta_PV_heating_swr :
            n_seeds_friction.append(i)
            # if delta_PV_Fx >= delta_PV_Fy :
            #     n_seeds_friction_Fx.append(i)
            # else :
            #     n_seeds_friction_Fy.append(i)
        else :
            if np.nansum(PV_dot_heating_LWR_interp) > 0:
                n_seeds_heating_lwr.append(i)
            else:
    ...
    # same but do determine which process destroys the most PV
...
print("Number of heating dominant trajectories :", len(n_seeds_heating))
print("Number of friction dominant trajectories :", len(n_seeds_friction))
print("Number of latent heating dominant trajectories :", len(n_seeds_heating_latent))
print("Number of radiative heating dominant trajectories :", len(n_seeds_heating_rad))
print("Number of SWR heating dominant trajectories :", len(n_seeds_heating_swr))
print("Number of LWR heating dominant trajectories :", len(n_seeds_heating_lwr))
print("Number of LWR heating dominant trajectories (destroying):", len(n_seeds_heating_lwr_neg))
# print("Number of Fx dominant trajectories :", len(n_seeds_friction_Fx))
# print("Number of Fy dominant trajectories :", len(n_seeds_friction_Fy))
...
traj0 = traj.isel(time_ind = time_ind)#.sel(n_seeds =n_seeds_heating_lwr_neg)
PV_dot_heating_lat0 = (PV_dot_heating_list - PV_dot_heating_LWR_list - PV_dot_heating_SWR_list)[:, time_ind]
traj0 = traj0.where(PV_dot_heating_lat0 * 3600 * 1e6 >= 0.15 )
...
# ax1.scatter(traj0['lon'], traj0['lat'], c = PV_dot_heating_LWR_list[:, time_ind], cmap = 'vanimo', vmin = -0.5, vmax = 0.5, marker = '+', linewidths = 1, transform=ccrs.PlateCarree(), alpha = 0.7)#, alpha = 0.3)
# plt.scatter(traj0['lat'].mean('n_seeds'), traj0['P'].mean('n_seeds'), color = 'orange', marker = '+', linewidths = 15)

ax1.plot([lon_min_cs, lon_min_cs], [lat_min_cs, lat_max_cs,], color = 'red', transform=ccrs.Geodetic(), linewidth = 2)
ax1.plot([lon_min_cs, lon_max_cs,], [ lat_max_cs, lat_max_cs], color = 'red', transform=ccrs.Geodetic(), linewidth = 2)
ax1.plot([ lon_max_cs, lon_max_cs], [ lat_max_cs, lat_min_cs], color = 'red', transform=ccrs.Geodetic(), linewidth = 2)
ax1.plot([ lon_max_cs, lon_min_cs], [ lat_min_cs, lat_min_cs], color = 'red', transform=ccrs.Geodetic(), linewidth = 2)
...
# Add other data variables
# Add other data variables
theta_dot_lat_cs = theta_dot_lat.sel(time=time_cs).sel(longitude=slice(lon_min_cs, lon_max_cs),    level=slice(lev_min_cs, lev_max_cs),
                                                     latitude=slice(lat_max_cs, lat_min_cs)).mean('latitude', skipna = False)
im = theta_dot_lat_cs.plot.contourf(ax=ax2, add_colorbar=False, levels = np.arange(-1.2,1.21, 0.2), extend = 'both' )
# im = theta_dot_lat.sel(time=time_cs).sel(longitude=slice(lon_min_cs, lon_max_cs),    level=slice(lev_min_cs, lev_max_cs)).sel(latitude=lat_max_cs, method = 'nearest').plot.contourf(
...
Pv_contours = (PV_pressure * 1e6).sel(time=time_cs).sel(longitude=slice(lon_min_cs, lon_max_cs),    level=slice(lev_min_cs, lev_max_cs),
                                                     latitude=slice(lat_max_cs, lat_min_cs)).mean(
    'latitude', skipna = False).plot.contour(
...
clouds = all_data_pressure['cldr'].sel(time=time_cs, method='nearest').sel(longitude=slice(lon_min_cs, lon_max_cs),    level=slice(lev_min_cs, lev_max_cs),
                                                     latitude=slice(lat_max_cs, lat_min_cs)).mean(
    'latitude', skipna = False).plot.contourf(
    ax=ax2, add_colorbar=False, levels = np.arange(0.1,1.1,0.1), alpha = 0.2, cmap = 'Greys')



CSu = v.sel(time=time_cs,  method='nearest').sel(longitude=slice(lon_min_cs,lon_max_cs),level=slice(lev_min_cs, lev_max_cs),
                                                                    latitude=slice(lat_max_cs, lat_min_cs)).mean('latitude', skipna = False).plot.contour(ax=ax2,
                                                                                                                  add_colorbar=False,
...
traj0 = traj.isel(time_ind = time_ind)#.sel(n_seeds =n_seeds_heating_lwr_neg)
#### Selection of trajectories with significant LWR heating destroying PV
PV_dot_heating_LWR0 = PV_dot_heating_LWR_list[:, time_ind]
traj0 = traj0.where(PV_dot_heating_LWR0 * 3600 * 1e6 >= 0.15 )
...
ax2.scatter(traj0['lon'], traj0['P'] / 100, color = 'black',  marker = '+', linewidths = 1, alpha = 0.5)#, alpha = 0.3)
# scatter_traj = ax2.scatter(traj0['lon'], traj0['P'] / 100, c = PV_dot_heating_LWR_list[:, time_ind], cmap = 'vanimo', vmin = -0.5, vmax = 0.5, marker = '+', linewidths = 1, alpha = 0.7)#, alpha = 0.3)
...
#
# cbar_ax_traj = fig.add_axes([0.3, 0.1, 0.4, 0.9])
# cbar_traj = fig.colorbar(scatter_traj, cax=cbar_ax_traj, orientation='horizontal')#,label = 'Potential vorticity [pvu]')#, fontsize = 20)
# # cbar.set_label(r'$\dot{\theta}_{long wave}$ [$K.h^{-1}$]')#, size = 20)
...
plt.savefig(f'{path_plots}/latent_meridional_article_{dataset}_{cyc}.png', bbox_inches='tight', transparent = True)
plt.show()
