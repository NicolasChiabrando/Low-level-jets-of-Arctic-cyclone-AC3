...
im = (pv * 1e6).sel( time = time, level = pres, method = 'nearest' ).plot.contourf(ax=ax, add_colorbar = False,  transform=ccrs.PlateCarree(), levels = np.arange(-4, 4.1, 1.),   extend = 'both') # levels =[-0.8, 0, 0.8, 1.6, 3.2, 4.8, 5.6])# )#, cmap = 'plasma')
...
CS = U.plot.contour(ax=ax, add_colorbar = False,  transform=ccrs.PlateCarree(), cmap ='green', levels = np.arange(15,31, 5), linewidths = 2)
plt.clabel(CS, inline=True, colors = 'green', fontsize = 12)#, fmt = "%.2f")
ax.legend()
ax.set_title('')
...
im = (pv * 1e6).sel(time=time,  method='nearest').sel(longitude=slice(lon_min_cs,lon_max_cs),level=slice(lev_min_cs, lev_max_cs),
    ...
    ax=ax, add_colorbar=False,levels=np.arange(-4, 4.1, 1.), extend = 'both')  # levels =[-0.8, 0, 0.8, 1.6, 3.2, 4.8, 5.6])# )#, cmap = 'plasma')
...
plt.clabel(CSt, inline=True, colors='black', fontsize=12)#, fmt="%.2f")
plt.clabel(CSu, inline=True, colors='green', fontsize=12)#, fmt="%.2f")
ax.set_title('')
