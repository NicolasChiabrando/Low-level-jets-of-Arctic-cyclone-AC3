...
#             if traj_ini.isel(time_ind=time_ind0).sel(n_seeds=i)['lat'] <= 72.5:
#                 if traj_ini.isel(time_ind=time_ind0).sel(n_seeds=i)['lat'] >= 71.5:
...
im = (PV_pressure * 1e6).sel( time = time_cs, level = pres, method = 'nearest' ).plot.contourf(ax=ax1, add_colorbar = False,  transform=ccrs.PlateCarree(), levels = pv_lev, cmap = newcmp,   extend = 'both') # levels =[-0.8, 0, 0.8, 1.6, 3.2, 4.8, 5.6])# )#, cmap = 'plasma')
...
traj0 = traj0.where(traj0['P'] >= (pres - 25) * 100 )
traj0 = traj0.where(traj0['P'] <= (pres + 25) * 100 )

ax1.scatter(traj0['lon'], traj0['lat'], color = 'black',  transform=ccrs.PlateCarree(), marker = '+', alpha = 0.5, s = s, linewidths = linewidths_p)#, alpha = 0.3)
...
Pv_contours = (all_data_pressure['theta']).sel(time=time_cs, level = pres, method = 'nearest'  ).plot.contour(
    ax=ax1, add_colorbar=False, transform=ccrs.PlateCarree(), colors='black', linewidth = 3, levels = np.arange(260,320,2))  # levels =[-0.8, 0, 0.8, 1.6, 3.2, 4.8, 5.6])# )#, cmap = 'plasma')
...
# Add other data variables
# Add other data variables
im = (PV_pressure * 1e6).sel(time=time_cs,  method='nearest').sel(longitude=slice(lon_min_cs,lon_max_cs),level=slice(lev_min_cs, lev_max_cs),
                                                                    latitude=slice(lat_max_cs, lat_min_cs)).mean('longitude', skipna = False).plot.contourf(
    ax = ax2, add_colorbar = False,levels = pv_lev, cmap = newcmp, extend = 'both')  # levels =[-0.8, 0, 0.8, 1.6, 3.2, 4.8, 5.6])# )#, cmap = 'plasma')



CSt = all_data_pressure['theta'].sel(time=time_cs,  method='nearest').sel(longitude=slice(lon_min_cs,lon_max_cs),level=slice(lev_min_cs, lev_max_cs),
...
# traj0 = traj.isel(time_ind = time_ind).sel(n_seeds =n_seeds_heating_lwr)
traj1 = traj.isel(time_ind = time_ind)#.sel(n_seeds =n_seeds_heating_lwr_neg)
# PV_dot_heating_LWR0 = PV_dot_heating_LWR_list[:, time_ind0]
