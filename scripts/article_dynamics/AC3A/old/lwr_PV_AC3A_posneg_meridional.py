...
#     plt.plot(t_list, PV_dot_heating_LWR_list[i] * 1e6 * 3600, alpha = 0.3)
# plt.plot(t_list, (np.nanmean(PV_dot_heating_LWR_list, axis = 0) )* 1e6 * 3600, color = 'black', linewidth = 2)
# plt.xlabel('Time')
# plt.ylabel(r'$PV \ dot_{LWR}$ [pvu/h]')
# plt.title(f'{cyc}')
# plt.show()
#
# fig0 = plt.figure(0)
# for i in range(np.size(traj['n_seeds'])):
#     plt.plot(t_list, PV_dot_heating_SWR_list[i] * 1e6 * 3600, alpha = 0.3)
# plt.plot(t_list, (np.nanmean(PV_dot_heating_SWR_list, axis = 0) )* 1e6 * 3600, color = 'black', linewidth = 2)
# plt.xlabel('Time')
# plt.ylabel(r'$PV \ dot_{SWR}$ [pvu/h]')
# plt.title(f'{cyc}')
# plt.show()
#
#
# fig0 = plt.figure(0)
# for i in range(np.size(traj['n_seeds'])):
#     plt.plot(t_list,  (PV_dot_heating_LWRCS_list[i] ) * 1e6 * 3600, alpha = 0.3)
# plt.plot(t_list, (np.nanmean(PV_dot_heating_LWRCS_list, axis = 0) )* 1e6 * 3600, color = 'black', linewidth = 2)
# plt.xlabel('Time')
# plt.ylabel(r'$PV \ dot_{LWR CS}$ [pvu/h]')
# plt.title(f'{cyc}')
# plt.show()
#
# fig0 = plt.figure(0)
# for i in range(np.size(traj['n_seeds'])):
#     plt.plot(t_list, (PV_dot_heating_SWRCS_list[i] )* 1e6 * 3600, alpha = 0.3)
# plt.plot(t_list, (np.nanmean(PV_dot_heating_SWRCS_list, axis = 0) )* 1e6 * 3600, color = 'black', linewidth = 2)
# plt.xlabel('Time')
# plt.ylabel(r'$PV \ dot_{SWR CS}$ [pvu/h]')
# plt.title(f'{cyc}')
# plt.show()
...
lon_min_cs = 0#0 #0#28 #
lon_max_cs =  20 #20 # 20  #50 #
lat_min_cs = 73.5
lat_max_cs = 74.5
time_ind = 2

# lon_min_cs = -15#0 #0#28 #
# lon_max_cs = 0  #20 # 20  #50 #
# lat_min_cs = 68.5
# lat_max_cs = 69.5
# time_ind = 12
...
theta_dot_lwr = all_data_pressure['MTTLWR'] * (1000 / all_data_pressure['level'])**kappa * 3600 #K / h
...
im = theta_dot_lwr.sel(time=time_cs, level= pres).plot.contourf(
    ax=ax1, add_colorbar=False,  extend = 'both', transform=ccrs.PlateCarree(), levels = np.arange(-1.2,1.21, 0.2))
...
# traj0 = traj0.where(PV_dot_heating_LWR0 * 3600 * 1e6 >= threshold )
#
# traj0 = traj0.where(traj0['P'] <= (pres + 10) * 100 )
# traj0 = traj0.where(traj0['P'] >= (pres - 10) * 100 )
#
# # ax1.scatter(traj0['lon'], traj0['lat'], c = PV_dot_heating_LWR_list[n_seeds_heating_lwr, time_ind] * 3600 * 1e6, cmap = 'BrBG', vmin = -1, vmax = 1,  marker = '+', linewidths = 1, transform=ccrs.PlateCarree())#, alpha = 0.3)
# ax1.scatter(traj0['lon'], traj0['lat'], color = 'black', marker = '+', linewidths = 1, transform=ccrs.PlateCarree(), alpha = 0.5)

traj0 = traj1.where(PV_dot_heating_LWR0 * 3600 * 1e6 >= threshold  )
...
# Add other data variables
# Add other data variables
im = theta_dot_lwr.sel(time=time_cs).sel(longitude=slice(lon_min_cs, lon_max_cs),    level=slice(lev_min_cs, lev_max_cs),
                                                     latitude=slice(lat_max_cs, lat_min_cs)).mean(
    'latitude', skipna = False).plot.contourf(
    ax=ax2, add_colorbar=False, extend = 'both' , levels = np.arange(-1.2,1.21, 0.2)) #levels = np.arange(-0.0003,0.00031,0.00005),
...
ax2.hlines(pres, lon_min_cs, lon_max_cs, linestyle = 'dashed', color = 'red')
...
traj0 = traj0.where(traj0['lat'] >= lat_min_cs )
traj0 = traj0.where(traj0['lat'] <= lat_max_cs )
ax2.scatter(traj0['lon'], traj0['P'] / 100, color = 'lime', marker = '+', alpha = 0.5,)#, alpha = 0.3)
...
ax2.scatter(traj0['lon'], traj0['P'] / 100, color = 'blue', marker = '_',  alpha = 0.5)#, alpha = 0.3)
...
plt.savefig(f'{path_plots}/lwr_posneg_meridional_article_{dataset}_{cyc}.png', bbox_inches='tight', transparent = True)
plt.show()
