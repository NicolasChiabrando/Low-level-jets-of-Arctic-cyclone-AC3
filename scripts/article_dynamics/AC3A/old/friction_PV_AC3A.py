...
im = Fx.sel(time=time_cs, level= pres).plot.contourf(
...
# Add other data variables
# Add other data variables
im = Fx.sel(time=time_cs).sel(longitude=slice(lon_min_cs, lon_max_cs),    level=slice(lev_min_cs, lev_max_cs),
                                                     latitude=slice(lat_max_cs, lat_min_cs)).mean(
