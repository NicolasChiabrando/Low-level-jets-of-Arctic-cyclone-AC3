...
theta_dot_lat = (all_data_pressure['MTTPM'] - all_data_pressure['MTTLWR'] )* (1000 / all_data_pressure['level'])**kappa
...
im = theta_dot_lat.sel(time=time_cs, level= pres ).plot.contourf(
    ax=ax1, add_colorbar=False, levels = np.arange(-0.0003,0.00031,0.00005), extend = 'both', transform=ccrs.PlateCarree() )

traj0 = traj.isel(time_ind = time_ind).sel(n_seeds =n_seeds_heating_latent)
traj0 = traj0.where(traj0['P'] >= (pres - 10) * 100 )
traj0 = traj0.where(traj0['P'] <= (pres + 10) * 100 )

ax1.scatter(traj0['lon'], traj0['lat'], color = 'black', marker = '+', linewidths = 1, transform=ccrs.PlateCarree())#, alpha = 0.3)
...
im = theta_dot_lat.sel(time = time_cs, method='nearest').sel(longitude=slice(lon_min_cs, lon_max_cs),    level=slice(lev_min_cs, lev_max_cs),
    ...
    ax=ax2, add_colorbar=False, levels = np.arange(-0.0003,0.00031,0.00005), extend = 'both')
...
ax2.scatter(traj0['lon'], traj0['P'] / 100, color = 'black', marker = '+', linewidths = 1)#, alpha = 0.3)
# plt.scatter(traj0['lat'].mean('n_seeds'), traj0['P'].mean('n_seeds'), color = 'orange', marker = '+', linewidths = 15)
clouds = all_data_pressure['cldr'].sel(time=time_cs, method='nearest').sel(longitude=slice(lon_min_cs, lon_max_cs),    level=slice(lev_min_cs, lev_max_cs),
                                                     latitude=slice(lat_max_cs, lat_min_cs)).mean(
    'latitude', skipna = False).plot.contourf(
...
CSu = v.sel(time=time_cs,  method='nearest').sel(longitude=slice(lon_min_cs,lon_max_cs),level=slice(lev_min_cs, lev_max_cs),
                                                                    latitude=slice(lat_max_cs, lat_min_cs)).mean('latitude', skipna = False).plot.contour(ax=ax2,
                                                                                                                  add_colorbar=False,
                                                                                                                  colors='green', levels=  np.arange(-25,26,5))
...
cbar.set_label(r'$\dot{\theta}_{heating - rad}$ [$K.s^{-1}$]')#, size = 20)
...
plt.savefig(f'{path_plots}/latent_meridional_article_PV_{dataset}_{cyc}.png', bbox_inches='tight', transparent = True)
plt.show()
