...
plt.plot(t_list, (np.nanmean(PV_dot_heating_LWR_list, axis = 0) )* 1e6 * 3600, color = 'black', linewidth = 2)
plt.vlines(t_list[time_ind], ymin = np.nanmin(PV_dot_heating_LWR_mean)* 1e6 * 3600, ymax = np.nanmax(PV_dot_heating_LWR_mean)* 1e6 * 3600 )
plt.xlabel('Time')
plt.ylabel(r'$PV \ dot_{LWR}$ [pvu/h]')
plt.title(f'{cyc}')
plt.show()
...
lon_min_cs = 9 #+ 19#0 #0#28 #
lon_max_cs = 13# + 19 #20 # 20  #50 #
lat_min_cs = 72
lat_max_cs = 78 #7
# time_ind = 2
...
#
#     if time_ind % 2 == 0:
#         time_cs = f'2022-08-16T{h_txt}:30'
#     else:
#         time_cs = f'2022-08-16T{h_txt}:30'
# elif cyc =='AC3B':
#     if time_ind > 7 :
...
im = theta_dot_lwr.sel(time=time_cs, level= pres, method = 'nearest' ).plot.contourf(
...
# Add other data variables
# Add other data variables
im = theta_dot_lwr.sel(time=time_cs, method = 'nearest' ).sel(longitude=slice(lon_min_cs, lon_max_cs),    level=slice(lev_min_cs, lev_max_cs),
                                                     latitude=slice(lat_max_cs, lat_min_cs)).mean(
