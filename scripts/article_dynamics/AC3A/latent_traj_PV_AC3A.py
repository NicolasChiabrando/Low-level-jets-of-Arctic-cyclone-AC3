...
PV_dot_heating_LWR_mean = np.nanmean(PV_dot_heating_lat_list, axis = 0)
...
plt.plot(t_list, (np.nanmean(PV_dot_heating_lat_list, axis = 0) )* 1e6 * 3600, color = 'black', linewidth = 2)
...
lon_min_cs = 6 +4#0 #0#28 #
lon_max_cs = 10+4  #20 # 20  #50 #
lat_min_cs = 70.5
lat_max_cs = 75.5
# time_ind =2
...
PV_dot_heating_LWR0 = PV_dot_heating_lat_list[:, time_ind0]
# traj0 = traj0.where(PV_dot_heating_LWR0 * 3600 * 1e6 >= threshold )
#
# traj0 = traj0.where(traj0['P'] <= (pres + 10) * 100 )
# traj0 = traj0.where(traj0['P'] >= (pres - 10) * 100 )
#
# # ax1.scatter(traj0['lon'], traj0['lat'], c = PV_dot_heating_lat_list[n_seeds_heating_lwr, time_ind] * 3600 * 1e6, cmap = 'BrBG', vmin = -1, vmax = 1,  marker = '+', linewidths = 1, transform=ccrs.PlateCarree())#, alpha = 0.3)
